...
macro_rules! errno {
    (
        ...
        pub fn strerror_impl(errno: Errno) -> &'static str {
            match errno {
                $( Errno::$name => $desc, )*
            }
        }
...
pub trait AsErrno: Error {
    fn errno(&self) -> Errno;
}

impl Error for Box<dyn AsErrno> {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        self.as_ref().source()
    }
}

impl<T: AsErrno + 'static> From<T> for Box<dyn AsErrno> {
    fn from(e: T) -> Self {
        Box::new(e)
    }
}
...
pub fn strerror(num: Errno) -> &'static str {
    // This function is generated inside the `errno!` macro.
    strerror_impl(num)
}

impl AsErrno for Infallible {
    fn errno(&self) -> Errno {
        match *self {}
    }
}

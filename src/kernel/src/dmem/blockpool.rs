...
const BLOCKPOOL_FILEOPS: VFileOps = VFileOps {
    read: |_, _, _| Err(GenericBlockpoolError::InvalidOperation.into()),
    write: |_, _, _| Err(GenericBlockpoolError::InvalidOperation.into()),
...
#[derive(Debug, Error)]
pub enum GenericBlockpoolError {
    #[error("Invalid operation")]
    InvalidOperation,
}

impl Errno for GenericBlockpoolError {
    fn errno(&self) -> NonZeroI32 {
        match self {
            GenericBlockpoolError::InvalidOperation => ENXIO,
        }
    }
}
